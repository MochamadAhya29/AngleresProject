...
class VideoAllAdapters(val context: Context, val videoList: ArrayList<VideoModel>) :
    RecyclerView.Adapter<VideoAllAdapters.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.clVideo.setOnClickListener {
            ...
            for (i in (position + 1) until videoList.size) {
                ...
                nextVideoList.add(
                    VideoListModel(
                        videoList.get(i).description,
                        videoList.get(i).thumb,
                        videoList.get(i).title,
                        videoList.get(i).url,
                        videoList.get(i).id
                    )
                )
            ...
            context.startActivity(Intent(context, VideoStreamPlayer::class.java).putExtras(bundle))
        }
    }
}